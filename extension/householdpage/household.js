...
$(function () { // Wait til all DOM loaded
	...
	$('#del').click(function () {
		...
		if (getArrVal(svc[btn.s], btn.n) != null) {
			$('#cbody').text("Are you sure to delete " + getArrVal(ip, btn.n) + " in Network " + btn.n + "? This can't be undo!")
	...
	$('#sav').click(function () {
		...
		if ($('#ip').val() != '') {
			...
			if (exist) {
				...
				$('#cbody').text("Are you sure to save " + $('#ip').val() + " as your new IP in Network " + btn.n + "? This can't be undo!");
				$('#cbtn').prop('hidden', false);
			}
		} else {
